...
class RegionT2I_AttnProcessor:
    ...
    def region_rewrite(self, attn, hidden_states, query, region_list, height, width):
        ...
        hidden_states = rearrange(
            hidden_states, "b (h w) c -> b h w c", h=feat_height, w=feat_width
        )
        ...
        replace_ratio = 1.0
        new_hidden_state[:, region_mask != 0, :] = (1 - replace_ratio) * hidden_states[
            :, region_mask != 0, :
        ]
        ...
        for region in region_list:
            ...
            attention_region = (
                einsum(
                    ...
                    query[:, start_h:end_h, start_w:end_w, :],
                    region_key,
                )
                * attn.scale
            )
            ...
            hidden_state_region = einsum(
                "b h w n, b n c -> b h w c", attention_region, region_value
            )
            new_hidden_state[:, start_h:end_h, start_w:end_w, :] += replace_ratio * (
                hidden_state_region
                / (
        ...
        if is_cross:
            ...
            for region in cross_attention_kwargs["region_list"]:
                if len(region[0].shape) == 4:
                    ...
                    region_value = attn.to_v(
                        region[0][:, self.cross_attention_idx, ...]
                    )
                else:
...
class RegionallyT2IAdapterPipeline(StableDiffusionAdapterPipeline):
    ...
    def _encode_region_prompt(
        ...
        if prompt_embeds is None:
            ...
            prompt_embeds = self.text_encoder(
                context_prompt_input_ids.to(device), attention_mask=None
            )[0]
            prompt_embeds = rearrange(
                prompt_embeds, "(b n) m c -> b n m c", b=batch_size
            )
            ...
            for idx, region in enumerate(region_list):
                ...
                region_embeds = self.text_encoder(
                    region_prompt_input_ids.to(device), attention_mask=None
                )[0]
                region_embeds = rearrange(
                    region_embeds, "(b n) m c -> b n m c", b=batch_size
                )
                ...
                region_neg_embeds = self.text_encoder(
                    region_negprompt_input_ids.to(device), attention_mask=None
                )[0]
                region_neg_embeds = (
                    (region_neg_embeds)
                    .view(batch_size, 1, seq_len, -1)
    ...
    def __call__(
        ...
        if keypose_adapter_input is not None:
            keypose_input = _preprocess_adapter_image(
                keypose_adapter_input, height, width
            ).to(self.device)
        ...
        if sketch_adapter_input is not None:
            sketch_input = _preprocess_adapter_image(
                sketch_adapter_input, height, width
            ).to(self.device)
        ...
        num_states = (
            len(keypose_adapter_state)
            if keypose_adapter_state is not None
            else len(sketch_adapter_state)
        )
        ...
        for idx in range(num_states):
            if keypose_adapter_state is not None:
                ...
                spatial_adaptor_weight = keypose_adaptor_weight * torch.ones(
                    *feat_keypose.shape[2:]
                ).to(feat_keypose.dtype).to(feat_keypose.device)
            ...
            if sketch_adapter_state is not None:
                ...
                spatial_adaptor_weight = sketch_adaptor_weight * torch.ones(
                    *feat_sketch.shape[2:]
                ).to(feat_sketch.dtype).to(feat_sketch.device)
