...
from mixofshow.models.edlora import (
    LoRALinearLayer,
    revise_edlora_unet_attention_controller_forward,
...
class EDLoRATrainer(nn.Module):
    def __init__(
        ...
        self.new_concept_cfg = self.init_new_concept(
            new_concept_token, initializer_token, enable_edlora=enable_edlora
        )
        ...
        if self.attn_reg_weight is not None:
            ...
            revise_edlora_unet_attention_controller_forward(
                self.unet, self.controller
            )  # support both lora and edlora forward
        else:
            revise_edlora_unet_attention_forward(
                self.unet
            )  # support both lora and edlora forward
    ...
    def set_finetune_cfg(self, finetune_cfg):
        ...
        if finetune_cfg["text_embedding"]["enable_tuning"]:
            ...
            if "weight_decay" in text_embedding_cfg:
                params_group.update(
                    {"weight_decay": text_embedding_cfg["weight_decay"]}
                )
            params_group_list.append(params_group)
            logger.info(f"optimizing embedding using lr: {text_embedding_cfg['lr']}")

        # 2. text encoder
        if finetune_cfg["text_encoder"]["enable_tuning"] and finetune_cfg[
            "text_encoder"
        ].get("lora_cfg"):
            ...
            params_group_list.append(
                {"params": params_list, "lr": text_encoder_cfg["lr"]}
            )
            logger.info(
        ...
        # 3. unet
        if finetune_cfg["unet"]["enable_tuning"] and finetune_cfg["unet"].get(
            "lora_cfg"
        ):
            ...
            logger.info(
                f"optimizing unet ({len(self.unet_lora)} LoRAs), using lr: {unet_cfg['lr']}"
            )
    ...
    def init_new_concept(
        self, new_concept_tokens, initializer_tokens, enable_edlora=True
    ):
        logger = get_logger("mixofshow", log_level="INFO")
        new_concept_cfg = {}
        ...
        assert len(new_concept_tokens) == len(
            initializer_tokens
        ), "concept token should match init token."

        for idx, (concept_name, init_token) in enumerate(
            zip(new_concept_tokens, initializer_tokens)
        ):
            if enable_edlora:
                ...
                num_new_embedding = 1
            new_token_names = [
                f"<new{idx * num_new_embedding + layer_id}>"
                for layer_id in range(num_new_embedding)
            ]

            num_added_tokens = self.tokenizer.add_tokens(new_token_names)
            assert num_added_tokens == len(
                new_token_names
            ), "some token is already in tokenizer"
            new_token_ids = [
                self.tokenizer.convert_tokens_to_ids(token_name)
            ...
            if init_token.startswith("<rand"):
                ...
                logger.info(
                    f"{concept_name} ({min(new_token_ids)}-{max(new_token_ids)}) is random initialized by: {init_token}"
                )
            else:
                # Convert the initializer_token, placeholder_token to ids
                init_token_ids = self.tokenizer.encode(
                    init_token, add_special_tokens=False
                )
                # print(token_ids)
                # Check if initializer_token is a single token or a sequence of tokens
                if len(init_token_ids) > 1 or init_token_ids[0] == 40497:
                    raise ValueError(
                        "The initializer token must be a single existing token."
                    )
                init_feature = token_embeds[init_token_ids]
                logger.info(
                    f"{concept_name} ({min(new_token_ids)}-{max(new_token_ids)}) is random initialized by existing token ({init_token}): {init_token_ids[0]}"
                )
    ...
    def forward(self, images, prompts, masks, img_masks):
        ...
        if self.enable_edlora:
            prompts = bind_concept_prompt(
                prompts, new_concept_cfg=self.new_concept_cfg
            )  # edlora
        ...
        if self.enable_edlora:
            encoder_hidden_states = rearrange(
                encoder_hidden_states, "(b n) m c -> b n m c", b=latents.shape[0]
            )  # edlora
    ...
    def cal_attn_reg(self, attention_maps, masks, text_input_ids):
        ...
        for text in text_input_ids:
            ...
            new_token_pos.append(
                [idx for idx in range(len(text)) if text[idx] in all_concept_token_ids]
            )
        ...
        for k, cross_map in attention_groups.items():
            ...
            cross_map = (
                cross_map.sum(-4) / cross_map.shape[-4]
            )  # e.g., 64 torch.Size([2, 64, 64, 77])
            cross_map = torch.stack(
                [
                    batch_map[..., batch_pos]
                    for batch_pos, batch_map in zip(new_token_pos, cross_map)
                ]
        ...
        for k, cross_map in attention_groups.items():
            ...
            gt_mask = F.interpolate(
                masks, size=map_subject.shape[1:], mode="nearest"
            ).squeeze(1)

            if self.reg_full_identity:
                loss_subject = F.mse_loss(
                    map_subject.float(), gt_mask.float(), reduction="mean"
                )
            else:
    ...
    def load_delta_state_dict(self, delta_state_dict):
        ...
        if (
            "new_concept_embedding" in delta_state_dict
            and len(delta_state_dict["new_concept_embedding"]) != 0
        ):
            ...
            if set(new_concept_tokens) != set(self.new_concept_cfg.keys()):
                logger.warning(
                    "Your checkpoint have different concept with your model, loading existing concepts"
                )
            ...
            for concept_name, concept_cfg in self.new_concept_cfg.items():
                ...
                token_embeds[concept_cfg["concept_token_ids"]] = token_embeds[
                    concept_cfg["concept_token_ids"]
                ].copy_(delta_state_dict["new_concept_embedding"][concept_name])

        # load text_encoder
        if (
            "text_encoder" in delta_state_dict
            and len(delta_state_dict["text_encoder"]) != 0
        ):
            load_keys = delta_state_dict["text_encoder"].keys()
            if hasattr(self, "text_encoder_lora") and len(load_keys) == 2 * len(
                ...
                for lora_module in self.text_encoder_lora:
                    for (
                        name,
                        param,
                    ) in lora_module.named_parameters():
                        logger.info(f"load: {lora_module.name}.{name}")
                        param.data.copy_(
                            delta_state_dict["text_encoder"][
                                f"{lora_module.name}.{name}"
                            ]
                        )
            else:
                for (
            ...
            if hasattr(self, "unet_lora") and len(load_keys) == 2 * len(self.unet_lora):
                ...
                for lora_module in self.unet_lora:
                    for (
                        ...
                        logger.info(f"load: {lora_module.name}.{name}")
                        param.data.copy_(
                            delta_state_dict["unet"][f"{lora_module.name}.{name}"]
                        )
            else:
                for (
                    name,
    ...
    def delta_state_dict(self):
        ...
        for concept_name, concept_cfg in self.new_concept_cfg.items():
            ...
            delta_dict["new_concept_embedding"][
                concept_name
