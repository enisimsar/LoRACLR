...
class EDLoRA_Control_AttnProcessor:
    ...
    def __init__(
        self, cross_attention_idx, place_in_unet, controller, attention_op=None
    ):
...
def revise_edlora_unet_attention_controller_forward(unet, controller):
    ...
    def change_forward(unet, count, place_in_unet):
        for name, layer in unet.named_children():
            if (
                layer.__class__.__name__ == "Attention" and "attn2" in name
            ):  # only register controller for cross-attention
                layer.set_processor(
                    EDLoRA_Control_AttnProcessor(count, place_in_unet, controller)
                )
                count += 1
            else:
...
class LoRALinearLayer(nn.Module):
    def __init__(self, name, original_module, rank=4, alpha=1):
        ...
        m = m[idxs] / 2
        with torch.no_grad():
            self.lora_down.weight = torch.nn.Parameter(
                torch.tensor(m, dtype=self.lora_down.weight.dtype)
            )
    ...
    def forward(self, hidden_states):
        hidden_states = self.original_forward(
            hidden_states
        ) + self.alpha * self.lora_up(self.lora_down(hidden_states))
        return hidden_states
