...
def load_new_concept(pipe, new_concept_embedding, enable_edlora=True):
    ...
    for idx, (concept_name, concept_embedding) in enumerate(
        new_concept_embedding.items()
    ):
        if enable_edlora:
        ...
        new_token_ids = [
            pipe.tokenizer.convert_tokens_to_ids(token_name)
        ...
        token_embeds[new_token_ids] = concept_embedding.clone().to(
            token_embeds.device, dtype=token_embeds.dtype
        )
...
def merge_lora_into_weight(original_state_dict, lora_state_dict, model_type, alpha):
    ...
    for k in new_state_dict.keys():
        ...
        if lora_up_name in lora_state_dict:
            ...
            lora_down_params = lora_state_dict[lora_down_name].to(
                original_params.device
            )
...
def convert_edlora(pipe, state_dict, enable_edlora, alpha=0.6):
    ...
    if (
        "new_concept_embedding" in state_dict
        and len(state_dict["new_concept_embedding"]) != 0
    ):
        pipe, new_concept_cfg = load_new_concept(
            pipe, state_dict["new_concept_embedding"], enable_edlora
        )

    # step 2: merge lora weight to unet
    unet_lora_state_dict = state_dict["unet"]
    pretrained_unet_state_dict = pipe.unet.state_dict()
    updated_unet_state_dict = merge_lora_into_weight(
        pretrained_unet_state_dict, unet_lora_state_dict, model_type="unet", alpha=alpha
    )
    ...
    updated_text_encoder_state_dict = merge_lora_into_weight(
        pretrained_text_encoder_state_dict,
