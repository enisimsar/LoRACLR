...
def draw_prompt(text, height, width, font_size=45):
    ...
    while font.font.getsize(text[:guess_count])[0][
        0
    ] + 0.1 * width < width - 0.1 * width and guess_count < len(
        text
    ):  # centerize
        guess_count += 1
...
def compose_visualize(dir_path):
    ...
    for filename in file_list:
        prompt, sample_args, index, suffix = osp.splitext(osp.basename(filename))[
            0
        ].split("---")
        ...
        if prompt not in info_dict["prompts"]:
            img_list.append(
                ToTensor()(
                    draw_prompt(prompt, height=height, width=width, font_size=45)
                )
            )
        ...
        img_list.append(img)
    assert (
        len(info_dict["sample_args"]) == 1
    ), "compose dir should contain images form same sample args."
    assert (
        len(info_dict["suffix"]) == 1
    ), "compose dir should contain images form same suffix."
