...
class LoraDataset(Dataset):
        ...
        for concept in concept_list:
            ...
            for x in Path(concept["instance_data_dir"]).iterdir():
                if x.is_file() and x.name != ".DS_Store":
                    ...
                    caption_path = (
                        os.path.join(caption_dir, f"{basename}.txt")
                    ...
                    if (
                        use_caption
                        and caption_path is not None
                        and os.path.exists(caption_path)
                    ):
                        with open(caption_path, "r") as fr:
                            line = fr.readlines()[0]
                            instance_prompt_image = self.process_text(
                                line, replace_mapping
                            )
                    else:
    ...
    def __getitem__(self, index):
        example = {}
        instance_image, instance_prompt, instance_mask = self.instance_images_path[
        ...
        instance_image, extra_args = self.instance_transform(
            instance_image, **extra_args
        )
