...
class PromptDataset(Dataset):
    ...
    def __init__(self, opt):
        ...
        self.prompts_to_generate = [
            (p, i)
            for i in range(1, self.num_samples_per_prompt + 1)
            for p in self.prompts
        ]
        self.latent_size = opt["latent_size"]  # (4,64,64)
        self.share_latent_across_prompt = opt.get(
            "share_latent_across_prompt", True
        )  # (true, false)
    ...
    def __getitem__(self, index):
        ...
        example["latents"] = torch.randn(
            self.latent_size, generator=torch.manual_seed(seed)
        )
        return example
