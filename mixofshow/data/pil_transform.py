...
class BILINEARResize(Resize):
    def __init__(self, size):
        super(BILINEARResize, self).__init__(
            size, interpolation=InterpolationMode.BILINEAR
        )
...
class PairCompose(nn.Module):
    ...
    def __call__(self, img, **kwargs):
        for t in self.transforms:
            if (
                len(inspect.signature(t.forward).parameters) == 1
            ):  # count how many args, not count self
                img = t(img)
            else:
    ...
    def forward(self, img, **kwargs):
        ...
        if random.random() < self.crop_p:
            if height > width:
                ...
                if "mask" in kwargs:
                    kwargs["mask"] = F.crop(
                        kwargs["mask"], 0, 0, width + crop_pos, width
                    )
            else:
                if "mask" in kwargs:
        ...
        if "mask" in kwargs:
            kwargs["mask"] = F.resize(
                kwargs["mask"], size=self.size - 1, max_size=self.size
            )
        ...
        if "mask" in kwargs:
            res_mask[start_y : start_y + new_height, start_x : start_x + new_width] = (
                kwargs["mask"][:new_height, :new_width]
            )
        ...
        if "mask" in kwargs:
            kwargs["mask"] = cv2.resize(
                kwargs["mask"], (self.size // 8, self.size // 8), cv2.INTER_NEAREST
            )
            kwargs["mask"] = torch.from_numpy(kwargs["mask"])
        kwargs["img_mask"] = cv2.resize(
            kwargs["img_mask"], (self.size // 8, self.size // 8), cv2.INTER_NEAREST
        )
