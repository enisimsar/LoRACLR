...
def visual_validation(accelerator, pipe, dataloader, current_iter, opt):
    ...
    for idx, val_data in enumerate(tqdm(dataloader)):
        ...
        for img, prompt, indice in zip(
            output, val_data["prompts"], val_data["indices"]
        ):
...
def test(root_path, args):
    ...
    for lora_alpha in opt["val"]["alpha_list"]:
        ...
        visual_validation(
            ...
            pipe,
            val_dataloader,
            f"validation_{lora_type}_{lora_alpha}",
            opt,
        )
        del pipe
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-opt", type=str, default="options/test/EDLoRA/EDLoRA_hina_Anyv4_B4_Iter1K.yml"
    )
