...
from mixofshow.utils.util import (
    MessageLogger,
    dict2str,
    reduce_loss_dict,
    set_path_logger,
...
def train(root_path, args):
    ...
    optimizer = torch.optim.AdamW(
        EDLoRA_trainer.get_params_to_optimize(), **train_opt["optim_g"]
    )
    ...
    val_dataloader = torch.utils.data.DataLoader(
        val_dataset, batch_size=valset_cfg["batch_size_per_gpu"], shuffle=False
    )

    # Prepare everything with our `accelerator`.
    EDLoRA_trainer, optimizer, train_dataloader, val_dataloader = accelerator.prepare(
        EDLoRA_trainer, optimizer, train_dataloader, val_dataloader
    )
    ...
    logger.info(
        f"  Instantaneous batch size per device = {opt['datasets']['train']['batch_size_per_gpu']}"
    )
    logger.info(
    ...
    original_embedding = copy.deepcopy(
        accelerator.unwrap_model(EDLoRA_trainer)
    ...
    while global_step < opt["train"]["total_iter"]:
        with accelerator.accumulate(EDLoRA_trainer):
            ...
            loss = EDLoRA_trainer(
                batch["images"], batch["prompts"], masks, batch["img_masks"]
            )
            loss_dict["loss"] = loss

            # get fix embedding and learn embedding
            index_no_updates = (
                torch.arange(len(accelerator.unwrap_model(EDLoRA_trainer).tokenizer))
                != -1
            )
            if not stop_emb_update:
                for token_id in accelerator.unwrap_model(
                    EDLoRA_trainer
                ).get_all_concept_token_ids():
        ...
        if accelerator.sync_gradients:
            ...
            token_embeds.data[index_no_updates, :] = original_embedding.data[
                index_no_updates, :
            ]
            ...
            concept_token_ids = accelerator.unwrap_model(
                EDLoRA_trainer
            ).get_all_concept_token_ids()
            loss_dict["Norm_mean"] = token_embeds[concept_token_ids].norm(dim=-1).mean()
            if stop_emb_update is False and float(
                loss_dict["Norm_mean"]
            ) >= train_opt.get("emb_norm_threshold", 5.5e-1):
    ...
    accelerator.wait_for_everyone()
    save_and_validation(
        accelerator, opt, EDLoRA_trainer, val_dataloader, "latest", logger
    )


def save_and_validation(
    accelerator, opt, EDLoRA_trainer, val_dataloader, global_step, logger
):
    enable_edlora = opt["models"]["enable_edlora"]
    lora_type = "edlora" if enable_edlora else "lora"
    save_path = os.path.join(
        opt["path"]["models"], f"{lora_type}_model-{global_step}.pth"
    )

    if accelerator.is_main_process:
        accelerator.save(
            {"params": accelerator.unwrap_model(EDLoRA_trainer).delta_state_dict()},
            save_path,
        )
        ...
        for lora_alpha in opt["val"]["alpha_list"]:
            ...
            pipe = pipeclass.from_pretrained(
                ...
                scheduler=DPMSolverMultistepScheduler.from_pretrained(
                    opt["models"]["pretrained_path"], subfolder="scheduler"
                ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-opt", type=str, default="options/train/EDLoRA/EDLoRA_hina_Anyv4_B4_Iter1K.yml"
    )
