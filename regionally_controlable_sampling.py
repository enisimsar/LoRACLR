...
from mixofshow.pipelines.pipeline_regionally_t2iadapter import (
    RegionallyT2IAdapterPipeline,
)
...
def build_model(pretrained_model, device):
    pipe = RegionallyT2IAdapterPipeline.from_pretrained(
    ...
    pipe.keypose_adapter = T2IAdapter.from_pretrained(
        "TencentARC/t2iadapter_openpose_sd14v1", torch_dtype=torch.float16
    ).to(device)
    pipe.sketch_adapter = T2IAdapter.from_pretrained(
...
def parse_args():
    ...
    parser.add_argument(
        "--pretrained_model",
        default="experiments/composed_edlora/anythingv4/hina+kario+tezuka+mitsuha+son_anythingv4/combined_model_base",
        type=str,
    )
